...
#Angle_Combsub = Angle_Comb.iloc[0:42,:]
#Angle_Combsub = Angle_Comb.iloc[[i % 200 == 0 for i in range(Angle_Comb.shape[0])],:]

#half = halftime(Angle_DF)
#angle_cossin = cossin(Angle_Comb)
...
#diff_time_samples = [int(x) for x in np.logspace(np.log10(50),np.log10(36500),100)]
...
#plt.plot(diff_time_samples,diff_time_samples_avg_error)
...
np.save("errors",error_data)


#X = np.repeat(t_num_samples, t_num_samples_num*len(t_timestep))
#Y = np.repeat(t_num_samples, t_num_samples_num*len(t_timestep))
#Z = np.repeat(t_timestep, t_num_samples_num**2)
#E = np.zeros(t_num_samples_num*t_num_samples_num*len(t_timestep))
...
#cmap=plt.hot()
#error_data = [[[variable_timesampledstepsize_avg_error(f, i, j, k) for i in t_num_samples]
#                               for j in t_num_samples] for k in t_timestep]
